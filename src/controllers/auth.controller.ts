...
export const refreshController = async (req: Request, res: Response) => {
    try {
        ...
        if (!token) {
            return res.status(401).json({ code: 401, message: "Refresh token missing" });
        }

        const data = await refreshToken(token);

        return res.status(200).json({ code: 200, message: "Token refreshed", data });
    } catch (err: any) {
        return res.status(401).json({ code: 401, message: err.message || "Invalid refresh token" });
    }
};

export const logoutController = async (req: AuthRequest, res: Response) => {
    try {
        if (!req.user) return res.status(401).json({ code: 401, message: "No user found" });

        const accessToken = req.headers.authorization?.split(" ")[1];
        const refreshToken = req.cookies?.refresh_token;

        if (!accessToken) return res.status(401).json({ code: 401, message: "No token provided" });
